...
abalone_codebook2_split <- initial_split(abalone_codebook2, prop = 0.75, strata = age)
...
```{r}
abalone_recipe <- recipe(age ~ ., data=abalone_codebook2_train) %>%
  step_dummy(all_nominal_predictors())%>% 
  step_interact(terms = ~ abalone_codebook2$type:abalone_codebook2$shucked_weight) %>%
  step_interact(terms = ~ abalone_codebook2$longest_shell:abalone_codebook2$diameter) %>%
  step_interact(terms = ~ abalone_codebook2$shucked_weight:abalone_codebook2$shell_weight) %>% step_normalize() %>% step_center()
abalone_recipe
```
...
#Error that comes up for me.
#Error in model.frame.default(object, data, xlev = xlev) : 
#invalid type (NULL) for variable 'abalone_codebook2$typeF'
```
